import{n as e}from"./assets/rolldown-runtime-DNdEV4Dn.js";import"./assets/styles-0BPvmdzI.js";import{n as t,t as n}from"./assets/vendor-CTp9fa9y.js";var r=e(t(),1),i=`55357047-4891ef3ad8f154bc0fd9bf3a9`,a=document.querySelector(`.input`),o=document.querySelector(`.btn`),s=document.querySelector(`.gallery`),c=document.querySelector(`.loader`);c.style.display=`none`;var l,u,d=1,f=9,p=0;o.addEventListener(`click`,e=>{e.preventDefault(),l=a.value.trim().replace(` `,`+`),d=1,s.innerHTML=``,c.style.display=`inline-block`,Promise.all([h(d),new Promise(e=>setTimeout(e,2e3))]).then(([e])=>{if(e.hits.length===0)return c.style.display=`none`,r.default.error({message:`Sorry, there are no images matching your search query. Please try again!`,position:`topRight`});p=Math.ceil(e.totalHits/f),g(e),m.style.display=`block`,d>=p&&(m.style.display=`none`),c.style.display=`none`}).catch(e=>{c.style.display=`none`,console.log(e)})});var m=document.querySelector(`.load-more`);m.style.display=`none`,m.addEventListener(`click`,()=>{d+=1,Promise.all([h(d),new Promise(e=>setTimeout(e,2e3))]).then(([e])=>{g(e),d>=p&&(m.style.display=`none`);let{height:t}=s.firstElementChild.getBoundingClientRect();window.scrollBy({top:t*4,behavior:`smooth`})}).catch(e=>{console.log(e)})});function h(e){return fetch(`
